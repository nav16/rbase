  ...
  class Table
    ...
    def open(name, options = {})
      ...
      loop do
        ...
        name = name.strip
        column = Columns::Column.column_for(type)
                                .new(name, options.merge(offset: offset,
                                                         size: size,
                                                         decimal: decimal))
        @columns << column
    ...
    attr_writer :count, :last_modified_on

    def update_header
      @last_modified_on = Date.today
      @file.pos = 1
      @file.write([last_modified_on.year % 100, last_modified_on.month,
                   last_modified_on.day].pack('ccc'))
